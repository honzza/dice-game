document.addEventListener("DOMContentLoaded", () => {
  ...
  //sets random values on all dice
  function updateAllDice(i) {
    for (let element of diceArray) {
      ...
      for (let j = 0; j < 9; j++) {
        ...
        dotDef[element.value - 1].includes(j)
  ...
  //calculates and displays score, set buttons behaviour
  function updateScore() {
	...
	//calculate score
    //valuesArray includes values of selected and not disabled dice
    for (let element of diceArray) {
      if (element.selected && !element.disabled)
        valuesArray.push(element.value);
    }
    //count object includes dice value as 'key' and its occurrence as 'value'
    valuesArray.forEach(function (i) {
      count[i] = (count[i] || 0) + 1;
    });
    //transfer count object entries to array
    const countArray = Object.entries(count);
    //comparing countArray items to scoreValue table and looking for score value
    countArray.forEach(([key, value]) => {
      if (scoreValues[key][value]) {
        rollScore += scoreValues[key][value];
        countScoringDice += value;
      }
    });
    //score for 1,2,3,4,5,6 dice values
  ...
  //show score
    ...
    tempScore < 350
      ? total.classList.add("lowscore")
      : total.classList.remove("lowscore");
  //set buttons behaviour
    roundButton.removeAttribute("disabled");
    for (let element of diceArray) {
      if (element.selected && !element.disabled) countSelectedEnabled++;
    }
    if (countSelected === 6 && rollScore > 0)
      roundButton.setAttribute("disabled", true);
